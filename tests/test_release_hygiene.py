...
class ReleaseHygieneTests(unittest.TestCase):
    ...
    def test_source_directory_cannot_be_used_as_output(self):
        ...
        with self.assertRaises(ValueError):
            self.builder.build("v0.2.5", self.builder.AE_SCRIPT.parent)
    ...
    def test_ae_package_contains_all_required_runtime_sidecars(self):
        self.builder.build("v0.2.5", self.output)
        archive_path = self.output / "CutBridge-AfterEffects-v0.2.5.zip"
    ...
    def test_ae_version_mismatch_is_rejected_before_output(self):
        ...
        source.write_text(source.read_text(encoding="utf-8").replace(
            'var PRODUCT_VERSION = "0.2.5";', 'var PRODUCT_VERSION = "0.2.2";'), encoding="utf-8")
    ...
    def test_artifact_symlink_cannot_overwrite_another_file(self):
        ...
        (self.output / "CutBridge-Blender-v0.2.5.zip").symlink_to(victim)
        with self.assertRaises(ValueError):
            self.builder.build("v0.2.5", self.output)
        self.assertEqual(victim.read_text(), "preserve me")

    def test_rc_build_uses_beta_prerelease_metadata(self):
        metadata = self.builder.build("v0.2.5-rc.1", self.output)
        self.assertEqual(metadata["version"], "0.2.5-rc.1")
        self.assertEqual(metadata["product_version"], "0.2.5")
        self.assertEqual(metadata["channel"], "beta")
        self.assertIs(metadata["prerelease"], True)
        self.assertTrue((self.output / "CutBridge-Blender-v0.2.5-rc.1.zip").is_file())
        self.assertTrue((self.output / "CutBridge-AfterEffects-v0.2.5-rc.1.zip").is_file())

    def test_development_build_uses_development_channel(self):
        metadata = self.builder.build("v0.2.5-dev.1", self.output)
        self.assertEqual(metadata["version"], "0.2.5-dev.1")
    ...
    def test_release_metadata_uses_visibility_neutral_distribution_boundary(self):
        metadata = self.builder.build("v0.2.5", self.output)
