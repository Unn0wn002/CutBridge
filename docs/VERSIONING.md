...
The numeric product version in Blender/AE stays `MAJOR.MINOR.PATCH`. A prerelease tag adds distribution metadata without changing the source product version: for example, source `0.2.4` may produce validation artifact `0.2.4-rc.1`.
...
The verified production notification index is `https://unn0wn002.github.io/cutbridge-distribution/cutbridge/release-index.json`, hosted separately from source control. v0.2.4 uses it for manual checks only; automatic startup update scheduling remains disabled. RC/beta/development GitHub Releases are still validation artifacts unless a matching entry is deliberately published to the production distribution.
