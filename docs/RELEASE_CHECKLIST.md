...
Current status note — 23 September 2026: stable v0.2.3 completed the checklist through authorized tag creation, GitHub Release publication, independent asset verification, and separate production D1 deployment. v0.2.4 frozen runtime candidate `candidate/v0.2.4-beta.1` remains `ef5911a83270a4ce8121741ada2d2726fa09ec29`; deterministic packaging, bounded native Blender/After Effects validation, focused Issue #82 clean-exit verification, protected `develop` → `main` promotion, and promoted-main CI are complete. This release-authorization branch now approves exactly `v0.2.4` / `stable` / `prerelease: false`; no v0.2.4 tag, GitHub Release, or production distribution entry exists. The next gate is merge of this authorization through protected `main` followed by exact-current-main `release-tag-eligibility` PASS. Tag creation and publication remain separate explicit steps. Completion for v0.2.3 does not weaken or pre-satisfy any v0.2.4 publication gate.
