...
Current status note — 23 September 2026: v0.2.4 completed protected promotion, exact authorization, release-tag eligibility, tag creation, GitHub Release workflow `35843821702`, and independent artifact verification on exact SHA `a393e409d19445c4090460b7e7b4716779161fa4`. Published v0.2.4 remains immutable. Production distribution intentionally remains on v0.2.3 because Issue #99 found stale pre-publication wording in the immutable v0.2.4 AE `INSTALL.md`. The v0.2.5 development baseline reconciles published-main history with the source fix, bumps product identity to 0.2.5, and restores `release-authorization.json` to `approved: false`. No v0.2.5 tag, GitHub Release, or production distribution entry is authorized.
