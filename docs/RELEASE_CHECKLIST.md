...
At the 20 September 2026 post-promotion reconciliation there are still no GitHub Releases or release tags. Protected `main` promotion and green promoted-main CI are verified, but controlled release-tag eligibility enforcement, exact release authorization, tag-triggered publication, release-asset download, and independent artifact/distribution verification remain separate gates. The release authorization gate is intentionally `approved: false` until those prerequisites are satisfied.
