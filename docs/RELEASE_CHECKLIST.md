...
Current status note — 23 September 2026: stable v0.2.3 completed the checklist through authorized tag creation, GitHub Release publication, independent asset verification, and separate production D1 deployment. Active v0.2.4 has a frozen beta.1 candidate at `ef5911a83270a4ce8121741ada2d2726fa09ec29`; deterministic packaging and the bounded exact-candidate native Blender/After Effects gate are recorded PASS for the tested scope, including the focused Issue #82 clean-exit rerun. `release-authorization.json` remains `approved: false`; no v0.2.4 tag, GitHub Release, or distribution entry exists. The next gate is promotion-readiness reconciliation, exact-head CI, and a deliberate protected `develop` → `main` PR. Completion for v0.2.3 does not weaken or pre-satisfy any v0.2.4 publication gate.
